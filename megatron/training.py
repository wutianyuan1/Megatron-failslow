...
def apply_batchsize_distribution(new_distribution: list[int]):
    args = get_args()
    args.batch_distribution = new_distribution
    set_args(args)   
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    while iteration < args.train_iters:
        ...
        if iteration % 10 == 0 and iteration < 100:
            # apply_batchsize_distribution([16 - iteration // 10, 16 + iteration // 10])
            apply_batchsize_distribution([10 + iteration // 10])           
            args = get_args()
            # print("iter:{} micr_bsz:{}".format(iteration, args.micro_batch_size))           
